...
7.  Next, some cleanup is done on the intermediate data frame, data_with_activity) to make it easier to turn it into a tidy data set.  The output of this step is the local data frame final_data.  Specifically, 
...
8. From the final state of the raw data (final_data) available at the end of step 7, the following steps are applied to turn this into a tidy data set that meets the principle of having a single variable per column & a single observation per row.  This script produces the long form of the tidy data set, meaning that there is 1 row per subject/activity/measurement and the row contains the mean of the values in the final_data set for that subject/activity/measurement combination.  Please see the codebook for specifics on the structure of the tidy data result.  The steps taken to transform the final_data data frame to a tidy data set are:
